...
@Component({
  ...
  template: `
    ...
    <div class="card" style="overflow:hidden">
      <app-grid-search-bar [value]="searchTerm()" (valueChange)="searchTerm.set($event)" placeholder="Search payables…" />
      <table class="table">
        <thead>
        ...
        <tbody>
          @for (r of filteredRows(); track r.supplierId) {
            ...
            <tr><td colspan="8" style="text-align:center;color:var(--muted)">{{ emptyGridMessage('No suppliers yet.') }}</td></tr>
          }
        </tbody>
      </table>
    </div>
