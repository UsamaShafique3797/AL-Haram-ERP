...
public class PurchaseInvoiceService : IPurchaseInvoiceService
{
    ...
    public PurchaseInvoiceService(AppDbContext db, IBranchScope branch)
    {
        _db = db;
        _branch = branch;
    }
    ...
    public async Task<Result<PurchaseInvoiceDto>> CreateAsync(SavePurchaseInvoiceRequest request, CancellationToken ct = default)
    {
        if (!_branch.CanUseGodown(request.GodownId))
            return Result<PurchaseInvoiceDto>.Failure("You can only create records for your assigned branch.");
