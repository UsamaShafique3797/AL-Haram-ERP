...
public class SupplierLedgerService : ISupplierLedgerService
{
    ...
    public SupplierLedgerService(AppDbContext db, IBranchScope branch)
    {
        _db = db;
        _branch = branch;
    }
    ...
    public async Task<SupplierLedgerDto> GetLedgerAsync(Guid supplierId, CancellationToken ct = default)
    {
        ...
        var opening = _branch.EffectiveGodownId is null ? supplier.OpeningBalance : 0m;
        var balance = opening;
        ...
        return new SupplierLedgerDto(
            ...
            opening, supplier.OpeningBalanceAsOf,
    ...
    public async Task<IReadOnlyList<PayableDto>> GetPayablesAsync(CancellationToken ct = default)
    {
        ...
        foreach (var s in suppliers)
        {
            ...
            var outstanding = opening + invoiced - paidAtPurchase - paid - returned;
            if (branchScoped && outstanding <= 0.0049m && invoiced == 0m) continue;

            result.Add(new PayableDto(s.Id, s.Name, s.Phone, opening,
